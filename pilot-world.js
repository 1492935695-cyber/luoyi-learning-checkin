...
import {PEOPLE} from './pilot-data.js?v=flow1';
import {MODELS,loadModelBytes,withDeadline,abortError} from './pilot-loading.js?v=flow1';
...
export class PilotWorld {
 ...
 build(ep){this.disposeGroup(this.root);this.clearTask();this.ep=ep;this.actors.clear();this.floaters=[];this.moves=[];this.departing=false;
  ...
  for(const [key,pos]of [[ep.characters[0],[0,0,-1.9]],[ep.characters[1],[-1.23,0,-1.0]],[ep.characters[2],[1.3,0,-1.5]]])this.character(key,pos);
 ...
 clearTask(){for(const g of this.targets.values()){const h=g.userData.hit;if(h){g.remove(h);h.geometry.dispose();h.material.dispose();delete g.userData.hit;}delete g.userData.target;}this.targets.clear();this.disposeGroup(this.taskRoot);this.disposeGroup(this.guideRoot);this.guides=[];this.sockets=[];for(const a of this.moves)if(a.task)a.resolve();this.moves=this.moves.filter(a=>!a.task);this.floaters=[];}
 ...
 move(o,to,d=.7,task=false){const destination=new T.Vector3(...to),previous=this.moves.find(a=>a.o===o);if(previous&&previous.to.distanceToSquared(destination)<.000001)return previous.promise;for(const old of this.moves.filter(a=>a.o===o))old.resolve();this.moves=this.moves.filter(a=>a.o!==o);if(o.position.distanceToSquared(destination)<.000001)return Promise.resolve();let resolve;const promise=new Promise(done=>resolve=done);this.moves.push({o,from:o.position.clone(),to:destination,t:0,d,task,resolve,promise});return promise;}
 ...
 collect(id,n,total=3){const g=this.targets.get(id);if(!g)return Promise.resolve();this.targets.delete(id);g.userData.target=null;this.lamps.push(g);return this.move(g,[(n-1-(total-1)/2)*.30,.75,-.55],.24);}
 ...
 async reward(id){const g=this.targets.get(id);if(g){if(['mail','clue'].includes(this.task?.kind)){const to=new T.Vector3();g.userData.hit.getWorldPosition(to);const w=this.task.kind==='mail'?this.task.goal[this.step]:'信',p=this.task.kind==='mail'?{天:'tiān',地:'dì',人:'rén'}[w]:'xìn';const letter=this.text(w,p,.38,.44);letter.position.set(0,.65,3.0);await this.move(letter,[to.x,to.y,to.z+.15],.24,true);const a=this.actors.get('brother');if(id==='人')a.g.rotation.y=-.13;}else{await this.move(g,id==='letter'?[0,.35,2.75]:[g.position.x,g.position.y+.22,g.position.z],.22,true);}}}
 ...
 toggleLamp(id){const g=this.targets.get(id);if(!g||!id.startsWith('lamp-'))return false;const removing=this.packed.has(id);if(removing)this.packed.delete(id);else this.packed.add(id);for(const[k,o]of this.targets){if(!k.startsWith('lamp-'))continue;const i=[...this.packed].indexOf(k);const to=i<0?o.userData.home.toArray():[(i-1)*.37,.76,-.55];this.move(o,to,.22);}return removing?'remove':'add';}
 ...
 shareCake(id){if(!id.startsWith('plate-'))return false;const plate=+id.slice(-1),free=this.cookiePlaces.indexOf(-1);let i,to;
  ...
  for(let k=0;k<4;k++){const owner=this.cookiePlaces[k],c=this.cookieObjects[k];if(owner<0)to=[-.76+k*.33,.15,1.3];else{const group=this.cookiePlaces.map((v,j)=>v===owner?j:-1).filter(j=>j>=0),rank=group.indexOf(k);to=[(owner? .85:-.85)+(rank%2-.5)*.34,.51,.05+(Math.floor(rank/2)-.5)*.34];}this.move(c,to,.22,true);}return true;}
