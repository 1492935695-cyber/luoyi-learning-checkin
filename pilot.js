import {PILOTS,PEOPLE,PILOT_VERSION,EXTRA_SPEECH} from './pilot-data.js?v=feedback2';
import {PilotWorld} from './pilot-world.js?v=feedback2';
...
async function respond(id,mine=token){const r=responses[id];if(!r)return;phase='feedback';$('feedback').hidden=false;$('pilot-app').classList.add('has-feedback');$('feedback-speaker').textContent=PEOPLE[r.who].name+'说';$('feedback-title').textContent=r.heading||'珞伊，看看这里';ruby($('feedback-learning'),r.learn||'',r.py||'');$('feedback-text').textContent=r.text;$('feedback-quantity').hidden=!r.count;$('feedback-number').textContent=r.count||'';$('feedback-dots').replaceChildren();for(let i=0;i<(r.count||0);i++){const dot=document.createElement('i');$('feedback-dots').append(dot);}if(!id.startsWith('retry-'))world.celebrate();const ok=await play(id);if(mine!==token||!ok)return;await waitActive(520,mine);if(mine===token)hideFeedback();}
...
async function sealPrompt(mine=token){locked=true;const target=task.goal[step];await play('find-'+findWords.indexOf(target));if(mine===token)locked=false;}
...
async function pick(id){if(phase!=='task'||paused||locked||!task)return;locked=true;const mine=token,b=task;let correct=false;
 ...
 if(!correct){
  ...
  else {world.hint();await play(b.kind==='mail'?'hint-mail':b.kind==='seals'?'find-'+findWords.indexOf(b.goal[step]):'hint-pronoun');}
 ...
 else if(b.kind==='mail'||b.kind==='seals'){const found=b.goal[step];await world.reward(id);if(mine!==token)return;await respond('good-word-'+readWords.indexOf(found),mine);step++;if(mine!==token)return;if(step<b.goal.length){world.setup(b,step);instructions(b);phase='task';if(b.kind==='seals')await sealPrompt(mine);else locked=false;return;}}
...
async function help(){if(phase!=='task'||paused||locked)return;const mine=token;locked=true;event('使用提示');world.hint();let id=task.kind==='bridge'?'hint-bridge':task.kind==='mail'?'hint-mail':task.kind==='seals'?'find-'+findWords.indexOf(task.goal[step]):['pronoun','receive'].includes(task.kind)?'hint-pronoun':task.kind==='return'?'hint-return':task.kind==='add'?'hint-add':'hint-load';await play(id);if(mine===token)locked=false;}
...
async function completeEpisode(){phase='finish';task=null;instructions(null);save[ep.id].done=true;event('完成故事');$('finish-title').textContent=ep.id==='math'?'珞伊，小灯车出发啦！':'珞伊，信都送到啦！';$('finish-text').textContent=ep.oral;$('finish-words').hidden=ep.id!=='chinese';$('finish-words').replaceChildren();if(ep.id==='chinese')for(const w of readWords){const item=document.createElement('span');ruby(item,w,py[w]);$('finish-words').append(item);}$('finish').showModal();await play('done-'+ep.id);}
function makeReport(){
 const lines=['珞伊的故事手记',new Date().toLocaleDateString('zh-CN'),''];
 const names={c08:'“我”指正在说话的朋友',c09:'换人说话后，“我”指哥哥',c11:'接收说给珞伊的“你”',c12:'“他”指第三个人',c15:'六字听音找字（带拼音）',m04:'按要求装三盏灯',m06:'灯换位置，数量不变',m09:'三盏添一盏成为四盏',m11:'四盏添一盏成为五盏',m14:'为四个灯座配灯'};
 for(const s of ['chinese','math']){
  const e=PILOTS[s],d=save[s];lines.push(`${e.subject}｜${e.title}`,`教材：${e.lesson} 第${e.pages}页${s==='math'?'选练':''}`,`故事状态：${d.done?'已看完（可重新体验）':d.events.length?'进行中':'尚未开始'}`);
  for(const b of e.beats.filter(b=>b.kind)){
   ...
   if(b.id==='c15'&&done&&!completed.some(x=>x.version===PILOT_VERSION))state='上版完成记录；本版六字任务尚未完整观察';
...
$('listen').onclick=async()=>{if(paused||!ep)return;if(['talk','feedback'].includes(phase)&&cue){audio.currentTime=0;audio.play().catch(()=>{$('audio-retry').hidden=false;});return;}if(locked)return;const mine=token;locked=true;if(task)event('重听要求');await play(task?.kind==='seals'?'find-'+findWords.indexOf(task.goal[step]):task?.id||ep.beats[index].id);if(mine===token)locked=false;};
